...
def _create_checkout_redirects_to_web(customer_id, connected_account_id, user_id,
                                      ...
                                      test_mode, web_origin="https://web.nutmegapp.com"):
    stripe.api_key = Secrets.STRIPE_KEY_TEST if test_mode else Secrets.STRIPE_KEY

    session = stripe.checkout.Session.create(
        success_url="{}/match/{}?payment_outcome={}".format(web_origin, match_id, "success"),
        cancel_url="{}/match/{}?payment_outcome={}".format(web_origin, match_id, "cancel"),
...
def _create_checkout_session_with_deep_links(customer_id, connected_account_id, user_id,
    ...
    session = stripe.checkout.Session.create(
        success_url=build_dynamic_link('{}/match/{}?payment_outcome={}'
                                       .format(web_origin, match_id, "success")),
        cancel_url=build_dynamic_link('{}/match/{}?payment_outcome={}'
                                      .format(web_origin, match_id, "cancel")),
        payment_method_types=["card", "ideal"],
        line_items=[
            {"price": price_id, "quantity": 1}
        ],
        ...
        mode="payment",
        customer=customer_id,
        metadata={"user_id": user_id, "match_id": match_id, "organizer_id": organizer_id, "is_test": str(test_mode).lower()}
    )
    return session
