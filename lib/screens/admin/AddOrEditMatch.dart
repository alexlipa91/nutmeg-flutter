        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              if (match != null)
                Row(children: [RoundedButton("CANCEL MATCH", () {
                  CoolAlert.show(context: context,
                      type: CoolAlertType.confirm,
                      text: "This is going to mark the match as CANCELED, send a push notification to all users currently joining and issue a credit refund (IMPLEMENT THIS).");
                })]),
              if (match != null)
                Row(children: [RoundedButton("VIEW SUBSCRIPTIONS", () async {
                  await Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => SubscriptionsMatchDetails(context.watch<MatchesChangeNotifier>().getMatch(match.documentId))));
              ...
              child: Column(
                ...
                children: [
                  ...
                  DropdownButton<SportCenter>(
                    ...
                    items: sportCenters.map<DropdownMenuItem<SportCenter>>(
                        (SportCenter value) {
                          ...
                          try {
                            if (match == null) {
                              var shouldAdd = await CoolAlert.show(
                                context: context,
                                type: CoolAlertType.confirm,
                                text: "This is going to add a new match. Are you sure?",
                              ...
                              if (shouldAdd) {
                                ...
                                assert(priceController.value.text != null, "specify price");

                                var newMatchId = await MatchesFirestore.addMatch(new Match(
                                    dateTime,
                                    sportCenterId,
                                    sport,
                                    maxPlayers,
                                ...
                                CoolAlert.show(
                                    ...
                                    text: "Success! Added match with id " + newMatchId);
                              }
                            } else {
                              ...
                              var shouldUpdate = await CoolAlert.show(
                                  context: context,
                                  type: CoolAlertType.confirm,
                                  text: "This is going to update the match with id " + match.documentId + ". Are you sure?",
                                  onConfirmBtnTap: () => Navigator.pop(context, true),
                                  onCancelBtnTap: () => Navigator.pop(context, false),
                              );
                              if (shouldUpdate) {
